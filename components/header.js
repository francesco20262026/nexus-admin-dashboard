/* header.js — inject the top header bar without document.write()
   Finds the #main-header placeholder in the DOM (or the .shell element) and
...
(function () {
  ...
  // Only inject if not already present (guards re-injection on SPA navigations)
  if (!document.getElementById('main-header')) {
    var slot = document.getElementById('header-slot');
    if (slot) {
      slot.outerHTML = headerHTML;
    } else {
      // Fallback: find the .shell element and prepend header as second child
      // (after aside.sidebar). This matches the original document.write approach.
